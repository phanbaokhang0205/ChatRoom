...
class Option_Buttons(ctk.CTkFrame):
    def __init__(self, container):
        ...
        self.user_list_btn = option_button_item(
            container=self,
            text="User List",
            commands=lambda: show_frame(app.frames['user_frame'], self.user_list_btn)  # Hiển thị Main_Content
        )
        self.noti_btn = option_button_item(
            container=self,
            text="Notification",
            commands=lambda: show_frame(app.frames['noti_frame'], self.noti_btn)  # Hiển thị Main_Content
        )
        self.image_mng = option_button_item(
            container=self,
            text="Image Manager",
            commands=lambda: show_frame(app.frames['noti_frame'], self.image_mng)  # Hiển thị Main_Content
        )
        self.file_mng = option_button_item(
            container=self,
            text="Files Manager",
            commands=lambda: show_frame(app.frames['noti_frame'], self.file_mng)  # Hiển thị Main_Content
        )
        self.media_mng = option_button_item(
            container=self,
            text="Media Manager",
            commands=lambda: show_frame(app.frames['noti_frame', self.media_mng])  # Hiển thị Main_Content
        )
        

        self.user_list_btn.grid(row=1, sticky='ew', padx=(40, 40), pady=(60, 0))
        self.noti_btn.grid(row=2, sticky='ew', padx=(40, 40), pady=40)
...
class Title_Table(ctk.CTkFrame):
    def __init__(self, container):
        ...
        self.columnconfigure(0, minsize=30)  # Cột ID có kích thước tối thiểu là 50px
        self.columnconfigure(2, minsize=30)  # Cột Tuổi (Age) có kích thước tối thiểu là 50px
...
# ----------------------Noti Frame----------------------------------------------------
class Notification_Frame(ctk.CTkFrame):
    def __init__(self, container):
        super().__init__(container,fg_color=CYAN, corner_radius=0)
...
if __name__ == "__main__":
    ...
    show_frame(app.frames['user_frame'], button_frame.user_list_btn)
